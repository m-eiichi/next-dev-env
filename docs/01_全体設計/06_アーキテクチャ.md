...
| 処理 | 入口 | 理由 |
| ---- | ---- | ---- |
| 画面の表示に使うデータの取得 | **Server Component**（`page.tsx` など） | サーバー内で直接ユースケースを呼べるので、API を作って HTTP で往復する必要がない |
| データの更新 | **Server Action** | フォームから直接呼べ、更新後のキャッシュの再検証も同じ場所で書ける |
| 外部から呼ばれる API、Webhook など | **Route Handler**（`route.ts`） | [04 データ取得・更新](../02_共通設計/04_データ取得・更新.md) の「4. Route Handler」の場合だけ使う |
...
| 層 | 役割 | 場所 |
| -- | ---- | ---- |
| プレゼンテーション層 | 画面の表示、ユーザーの操作を受け付ける | `src/app/`（page / layout など）、`src/components/`、`src/hooks/` |
| 入口（Entry Point） | 入力の受け取りとバリデーション、依存の組み立て、ユースケースの呼び出し、結果を画面や HTTP の形に変換 | Server Component、`actions.ts` / `src/actions/`、`route.ts` |
| アプリケーション層 | ユースケース（1 つの操作の手順）。認証・認可のチェック、ドメインの呼び出し、DTO への変換 | `src/application/` |
| ドメイン層 | 業務のルール。エンティティ、値オブジェクト、リポジトリのインターフェース | `src/domain/` |
| インフラストラクチャ層 | DB、認証サービス、外部 API の実装。DI コンテナ | `src/infrastructure/` |
...
| サーバー専用 | `src/application/`、`src/infrastructure/` のファイルの先頭に `import 'server-only'` を書き、Client Component から読み込めないようにする |

## 3. ディレクトリ構成

```
src/
├── app/                              # ルーティング（プレゼンテーション層 + 入口）
│   ├── (main)/
│   │   └── posts/
│   │       ├── page.tsx              # 一覧（Server Component で取得）
│   │       ├── actions.ts            # この画面だけで使う Server Action
...
│   └── globals.css
│
├── actions/                          # 複数の画面で使う Server Action（入口）
...
### 4.1 入口（Server Component / Server Action / Route Handler）
...
import { ListPostsUseCase } from "@/application/usecase/post/list-posts.usecase";
...
export default async function Page() {
  ...
  // DTO（プレーンなオブジェクト）が返るので、そのまま Client Component に渡せる
...
- 未ログインや権限なしの扱いは [01 認証・認可](../02_共通設計/01_認証・認可.md) に従う（`redirect()` / `notFound()` など）

#### 更新（Server Action）

```ts
// src/app/(main)/posts/actions.ts
"use server";

import { revalidateTag } from "next/cache";
import { z } from "zod";
import { container } from "@/infrastructure/di/container";
import { CreatePostUseCase } from "@/application/usecase/post/create-post.usecase";
import type { PostDto } from "@/application/dto/post/post.dto";
import { DomainError } from "@/domain/shared/domain-error";
import { UnauthorizedException } from "@/domain/auth/exception";
...
| ルール | 理由 |
| ------ | ---- |
| ユースケース、リポジトリ、DI コンテナを Client Component から import しない | サーバー専用のコードなので、`server-only` によりビルドエラーになる |
...
```ts
// src/application/usecase/post/create-post.usecase.ts
import "server-only";
import type { PostRepository } from "@/domain/post/repository";
import type { AuthService } from "@/domain/auth/service";
import { Post } from "@/domain/post/entity";
import { UnauthorizedException } from "@/domain/auth/exception";
import { toPostDto, type PostDto } from "@/application/dto/post/post.dto";
...
```ts
// src/application/dto/post/post.dto.ts
...
```ts
// src/domain/post/value-objects/post-title.ts
import { DomainError } from "../../shared/domain-error";
...
import type { PostRepository } from "@/domain/post/repository";
import type { AuthService } from "@/domain/auth/service";
import { PostRepositoryImpl } from "@/infrastructure/database/post/post-repository-impl";
import { AuthServiceImpl } from "@/infrastructure/auth/auth-service-impl";

/**
 * インターフェースと実装を結びつける。
 * 入口（Server Component / Server Action / Route Handler）からだけ使う。
 */
...
```ts
// src/infrastructure/database/post/post-repository-impl.ts
import "server-only";
import type { PostRepository } from "@/domain/post/repository";
import type { Post } from "@/domain/post/entity";
import type { PostId } from "@/domain/post/value-objects/post-id";
...
sequenceDiagram
    ...
    Page->>Container: postRepository() / authService()
    Container-->>Page: 実装
    Page->>UC: new ListPostsUseCase(repo, auth)
    Page->>UC: execute()
...
- 入口（Server Action / Route Handler）はユースケースを呼ぶだけなので、原則テストしない
- `async` の Server Component は Vitest でテストできない。表示部分を同期のコンポーネントに切り出すか、E2E テストで確認する

```ts
// src/application/usecase/post/create-post.usecase.test.ts
