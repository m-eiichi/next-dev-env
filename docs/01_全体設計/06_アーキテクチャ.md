...
| 入口（Entry Point） | 入力の受け取りとバリデーション、依存の組み立て、ユースケース・クエリの呼び出し、結果を画面や HTTP の形に変換 | `src/server/entry/`（`queries/`、`actions/`、`api/`） |
| アプリケーション層 | 更新（command）: ユースケース（1 つの操作の手順）。認証・認可のチェック、ドメインの呼び出し。読み取り（query）: クエリ。認証・認可のチェック、Query Service の呼び出し。どちらも結果は DTO | `src/server/application/`（`command/`、`query/`、`dto/`） |
| ドメイン層 | 業務のルール。エンティティ、値オブジェクト、リポジトリのインターフェース（更新に使う） | `src/server/domain/` |
| インフラストラクチャ層 | DB、認証サービス、外部 API の実装。リポジトリと Query Service の実装。DI コンテナ | `src/server/infrastructure/` |

- 更新（command）と読み取り（query）で、アプリケーション層から内側の道を分ける（CQRS。[ADR-016](../04_設計判断/ADR-016_CQRS.md)）。更新はエンティティとリポジトリを通って業務のルールを守り、読み取りはドメイン層を通らずに Query Service から DTO を受け取る
...
| アプリケーション層 | ドメイン層とアプリケーション層のインターフェースだけに依存する。インフラストラクチャ層（実装や DI コンテナ）は import しない。command と query は互いに import しない（共通で使うのは DTO とドメイン層だけ） |
| インフラストラクチャ層 | ドメイン層のインターフェース（リポジトリなど）と、アプリケーション層の Query Service のインターフェースを実装する |
...
| アプリケーション層の command（`application/command/`） | インフラストラクチャ層、入口、フロント、query（`application/query/`） |
| アプリケーション層の query（`application/query/`） | インフラストラクチャ層、入口、フロント、command（`application/command/`） |
| インフラストラクチャ層 | command（ユースケース）、入口、フロント（DTO と、query の Query Service のインターフェースは import してよい。Query Service を実装するため） |
| 入口 | フロント |
...
│   │   │   └── post/
│   │   │       └── post.dto.ts
│   │   ├── command/                  # 更新（CQRS の command）
│   │   │   └── post/
│   │   │       ├── create-post.usecase.ts
│   │   │       └── publish-post.usecase.ts
│   │   └── query/                    # 読み取り（CQRS の query）
│   │       └── post/
│   │           ├── post-query-service.ts   # 読み取り専用のインターフェース（DTO を返す）
│   │           ├── list-posts.query.ts
│   │           └── get-post.query.ts
│   │
│   ├── domain/                       # ドメイン層
...
│       ├── database/
│       │   └── post/
│       │       ├── post-repository-impl.ts      # 更新側（リポジトリの実装）
│       │       ├── post-query-service-impl.ts   # 読み取り側（Query Service の実装）
...
│ ── どちらにも属さない ────────────────
├── lib/                              # 層に属さないユーティリティ
│   └── utils.ts                      # shadcn/ui の CLI のための中継ぎ（cn()）
│
...
import { ListPostsQuery } from "@/server/application/query/post/list-posts.query";
import type { PostDto } from "@/server/application/dto/post/post.dto";

export async function listPosts(): Promise<PostDto[]> {
  // Composition Root: 依存を組み立ててクエリを作る（読み取りなので、リポジトリではなく Query Service を渡す）
  const query = new ListPostsQuery(container.postQueryService(), container.authService());
...
import { z } from "zod";
import { container } from "@/server/infrastructure/di/container";
import { CreatePostUseCase } from "@/server/application/command/post/create-post.usecase";
import type { PostDto } from "@/server/application/dto/post/post.dto";
...
export async function createPost(
  ...
  try {
    ...
    updateTag("posts"); // Server Action では updateTag（登録した人に古い一覧を見せない）
...
### 4.3 アプリケーション層（ユースケースとクエリ）

更新（command）と読み取り（query）で、置き場所と書き方を分ける（CQRS。[ADR-016](../04_設計判断/ADR-016_CQRS.md)）。

| | 更新（command） | 読み取り（query） |
| -- | --------------- | ----------------- |
| 呼び名 | ユースケース | クエリ |
| 置き場所 | `src/server/application/command/{エンティティ名}/{名前}.usecase.ts` | `src/server/application/query/{エンティティ名}/{名前}.query.ts` |
| 使うもの | エンティティ・値オブジェクトと、リポジトリ（ドメイン層） | Query Service（`src/server/application/query/{エンティティ名}/{エンティティ名}-query-service.ts` のインターフェース） |
| 業務のルール | エンティティに任せて守る | 持たない（読むだけ） |
| 認証・認可のチェック | 必ず行う | **必ず行う**（ドメイン層を通らなくても省かない） |
| 返すもの | DTO（Server Action が `ActionResult` に包む） | DTO |
| 呼ぶ入口 | `src/server/entry/actions/`（Server Action）、`src/server/entry/api/` | `src/server/entry/queries/`（取得の関数）、`src/server/entry/api/` |

- command と query は互いに import しない。共通で使うのは DTO とドメイン層だけ
- どちらも 1 つの操作につき 1 クラスにし、依存はコンストラクタで受け取る（下の「コンストラクタインジェクション」）
...
```ts
// src/server/application/command/post/create-post.usecase.ts
import "server-only";
...
> Next.js のドキュメントでいう **データアクセス層（DAL）** の役割（サーバーだけで動く、認可をチェックする、必要な項目だけの DTO を返す）は、ユースケース（更新）とクエリ（読み取り）が担う。[01 認証・認可](../02_共通設計/01_認証・認可.md) と [04 データ取得・更新](../02_共通設計/04_データ取得・更新.md) の「データアクセス層」はユースケースとクエリを指す。

#### 読み取り（query）: クエリと Query Service

**やること**

- 認証・認可のチェック
- Query Service を呼び、DTO をそのまま返す（エンティティは作らない）

```ts
// src/server/application/query/post/post-query-service.ts
import type { PostDto } from "@/server/application/dto/post/post.dto";

// 読み取り専用のインターフェース。実装はインフラストラクチャ層（「4.5」）
export interface PostQueryService {
  listByAuthorId(authorId: string, options: { limit: number; cursor?: string }): Promise<PostDto[]>;
  findById(id: string): Promise<PostDto | null>;
}
```

```ts
// src/server/application/query/post/list-posts.query.ts
import "server-only";
import type { AuthService } from "@/server/domain/auth/service";
import { UnauthorizedException } from "@/server/domain/auth/exception";
import type { PostDto } from "@/server/application/dto/post/post.dto";
import type { PostQueryService } from "./post-query-service";
...
export class ListPostsQuery {
  ...
  async execute(): Promise<PostDto[]> {
    ...
    // 2. Query Service から DTO をそのまま受け取る
    return this.postQueryService.listByAuthorId(user.id, { limit: 20 });
  }
}
```

- Query Service のインターフェースは、アプリケーション層（`query/`）に置く。返すのが DTO（アプリケーション層の型）なので、ドメイン層には置けない
- 読み取りと更新で同じ条件を使う場合（「公開済みのものだけ」など）は、条件をドメイン層の関数・定数にして、Query Service の実装とエンティティの両方から使う
...
| 差し替えやすい | DB や認証サービスを変えても、ユースケース・クエリは変更しなくてよい |
...
- リポジトリ、外部サービスのインターフェースを定義する（リポジトリは更新に使う。読み取りは Query Service が受け持つ。「4.3」）
...
// 更新に要るものだけを置く。一覧・検索・ページ分けは Query Service（「4.3」）
export interface PostRepository {
  save(post: Post): Promise<Post>;
  findById(id: PostId): Promise<Post | null>; // 更新の前に、今の状態を取り出すため
...
- アプリケーション層の Query Service のインターフェースを実装する（読み取り。DB の行から DTO を直接作る。結合、件数、ページ分けを SQL などで直接書いてよい）
- DB の行とエンティティの変換（更新側）
...
import "server-only";
import type { PostRepository } from "@/server/domain/post/repository";
import type { PostQueryService } from "@/server/application/query/post/post-query-service";
import type { AuthService } from "@/server/domain/auth/service";
import { PostRepositoryImpl } from "@/server/infrastructure/database/post/post-repository-impl";
import { PostQueryServiceImpl } from "@/server/infrastructure/database/post/post-query-service-impl";
...
export const container = {
  postRepository: (): PostRepository => new PostRepositoryImpl(), // 更新（command）
  postQueryService: (): PostQueryService => new PostQueryServiceImpl(), // 読み取り（query）
...
#### Query Service の実装（読み取り）

```ts
// src/server/infrastructure/database/post/post-query-service-impl.ts
import "server-only";
import type { PostDto } from "@/server/application/dto/post/post.dto";
import type { PostQueryService } from "@/server/application/query/post/post-query-service";
...
export class PostQueryServiceImpl implements PostQueryService {
  async listByAuthorId(
    ...
    // 画面に要る項目だけを取り、エンティティを通さずに DTO を作る
...
- 読み取りでは、エンティティを作らない。DB の行から DTO を直接作る
- 業務のルールは書かない。読み取りと更新で同じ条件を使う場合は、ドメイン層の関数・定数を使う（「4.3」）
...
sequenceDiagram
    ...
    participant Fn as listPosts（取得の関数）
    participant Container as DI Container
    participant Q as ListPostsQuery
    participant Auth as AuthService
    participant QS as PostQueryService
    participant DB as DB

    Page->>Fn: listPosts()
    Fn->>Container: postQueryService() / authService()
    Container-->>Fn: 実装
    Fn->>Q: new ListPostsQuery(queryService, auth)
    Fn->>Q: execute()
    Q->>Auth: getCurrentUser()
    Auth-->>Q: user
    Q->>QS: listByAuthorId(user.id, { limit: 20 })
    QS->>DB: SELECT（画面に要る項目だけ）
    DB-->>QS: rows
    QS-->>Q: PostDto[]（エンティティを通らない）
    Q-->>Fn: PostDto[]
    Fn-->>Page: PostDto[]
    Page->>Page: <PostList posts={dtos} /> を描画
```

### 5.2 更新（投稿の作成。command）
...
sequenceDiagram
    ...
    Repo-->>UC: Post
    UC-->>Action: PostDto
    Action->>Action: updateTag("posts")
    Action-->>Form: { ok: true, data }
```

## 6. テスト方針

層を分けているので、テストの中心は Next.js に依存しないドメイン層と、ユースケース・クエリになる。書き方のルールは [10 テスト](../02_共通設計/10_テスト.md) を参照。

| 層 | テストの内容 | 方法 | ツール |
| -- | ------------ | ---- | ------ |
| ドメイン層 | エンティティ、値オブジェクトのルール | そのまま単体テスト（依存がないのでモック不要） | Vitest |
| アプリケーション層（command） | ユースケースの手順、認可のチェック、DTO への変換 | リポジトリと AuthService をモックにして渡す | Vitest |
| アプリケーション層（query） | 認可のチェック、Query Service に渡す条件 | Query Service と AuthService をモックにして渡す | Vitest |
| インフラストラクチャ層 | DB とのやり取り、変換（リポジトリと Query Service の両方） | テスト用の DB を使う結合テスト | Vitest |
...
- 入口（取得の関数 / Server Action / Route Handler）はユースケース・クエリを呼ぶだけなので、原則テストしない
- `async` の Server Component は Vitest でテストできない。表示部分を同期のコンポーネントに切り出すか、E2E テストで確認する

```ts
// src/server/application/command/post/create-post.usecase.test.ts
...
| page や Server Action で DB に直接アクセスする | 認可のチェックや変換が散らばる | ユースケース（更新）・クエリ（読み取り）を通す |
...
| 読み取りのために、リポジトリに一覧・検索・ページ分けのメソッドを足す | リポジトリとエンティティが画面の都合でふくらむ | Query Service に足す（「4.3」） |
| クエリの中でエンティティを作る、業務のルールを書く | 読み取りと更新の役割が混ざる | 読み取りは DTO を直接受け取る。ルールは更新側（エンティティ）に書く |
| クエリで認証・認可のチェックを省く | ドメイン層を通らないので、ほかにチェックする場所がない | クエリの最初で必ずチェックする |
...
| 状況 | 省略してよいもの |
| ---- | ---------------- |
| 読むだけで、更新がないデータ（マスタの一覧など） | ドメイン層（エンティティ、値オブジェクト、リポジトリ）。読み取り（query）側だけを作る |
| ルールがほとんどない単純な登録・更新 | 値オブジェクト |

- 読み取りは、もともとエンティティを通らない（「4.3」）。更新がないデータは、ドメイン層に何も置かなくてよい

ただし、**認可のチェック**と **DTO で返すこと**は省略しない。
...
- [ADR-016 CQRS](../04_設計判断/ADR-016_CQRS.md)
...
| 2026-09-26 | アプリケーション層を更新（command）と読み取り（query）に分けた（CQRS。[ADR-016](../04_設計判断/ADR-016_CQRS.md)）。「2」「3」「4.3」「4.4」「4.5」「5」「6」「7」「8」を更新 | |
